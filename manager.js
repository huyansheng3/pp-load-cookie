...
document.addEventListener('DOMContentLoaded', function () {
  ...
  file.addEventListener('change', e => {
    ...
    var readFilePromiseArr = files.map(file => readFileAsync(file))
    console.time('readFile')

    Promise.all(readFilePromiseArr).then(cookiesArr => {
      console.log(cookiesArr)
      console.timeEnd('readFile')
      const cookiesList = cookiesArr.reduce((prev, curr) => {
      ...
      cookiesList.forEach((item, index) => {
        ...
        <tr>
          ...
          <td>
            ${item.cookies.substr(0, 20)}
          </td>

          <td>
            <input data-index="${index}" data-cookies="${item.cookies}" type="checkbox" />
          </td>
        </tr>
        `
      })

      $('#cookiesTable').append(html)

      $('#cookiesTable tr').click(function (event) {
        if (event.target.type !== 'checkbox') {
          $(':checkbox', this).trigger('click');
        }
      });

      $('#cookiesTable input[type=checkbox]').on('change', e => {
        if (e.target.checked) {
          var cookiesStr = e.target.dataset.cookies
          ...
          chrome.cookies.getAll({ url: `http://${host}` }, function (removeCookies) {
            for (var i in removeCookies) {
              removeCookie(removeCookies[i]);
            }

            cookies.forEach(cookie => {
              const [name, value] = cookie.split('=')
              if (!name.startsWith('cwrClyrK') && value !== undefined) {
                try {
                  chrome.cookies.set({
                    name,
                    value,
        ...
        str.split('======================================').forEach(str => {
          ...
          str.split('\n').forEach(line => {
            if (line.startsWith('Username:')) {
              name = line.replace('Username: ', '')
            }

            if (line.startsWith('Cookie:')) {
              value = line.replace('Cookie: ', '')
            }
          })
